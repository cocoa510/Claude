...
出力JSONには `diversity_constraint` フィールドも含まれる（G-4 判定結果）。
`diversity_constraint.blocked == true` の場合、`diversity_constraint.forbidden_class_names`
に列挙された class_name を**生成しない**こと。Redesign_v2_Plan §6.10 G-4 により、
連続 3 世代で同一 class_name を生成することは禁止されている（パターン多様性担保）。

`cluster_saturation.saturated_clusters` も併せて参照し、飽和済みクラスターを避けて生成すること。
...
> 4. `metadata.json` — 生成メタデータ（`direction_bias` + `display_name` フィールドを必ず含めること、display_name は `atlas.common.naming.format_display_name` で同 combo 内の最大 seq+1 を採番）
>
> ATLAS/CLAUDE.md と ATLAS/atlas/common/models.py を読んでインターフェースに準拠すること。
> 方向制約は不可侵の最優先制約であり、違反したコードは即座に破棄される。
>
> **G-4（多様性制約）**: コンテキスト `diversity_constraint.forbidden_class_names` に
> 含まれる class_name は使用禁止。別の class_name を選ぶこと（直近 3 世代連続同一を阻止）。
...
=== ATLAS 戦略生成完了 ===
世代ID: <generation_id>
表示名: <display_name>  (例: USDJPY-Breakout-Long-M15-006)
戦略タイプ: <戦略タイプ>
通貨ペア: <通貨ペア>
時間足: <時間足>
