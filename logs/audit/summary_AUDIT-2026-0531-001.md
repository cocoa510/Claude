...
**対象**:
- ATLAS `C:\data\works\FX\ATLAS` @ `74888095`
- FTS `C:\data\works\FX\fx_trading_system` @ `fca45f23`
...
### 検証後 severity 分布（2026-06-01 ユーザー指摘訂正反映）

| Severity | 件数 | 主な内容 |
|----------|------|---------|
| **CRITICAL** | **0** | ~~RE-505~~ ユーザー仕様確認で無効化 (週末ログ抑制仕様) |
| **HIGH** | **14** | post_fix_defect 5 + 新規 9 |
| MEDIUM | 30 | |
| LOW / INFO | 41 | |
| INVALID | 1 | DM-509 (前提誤り) |
| **合計** | **87** | |
...
1. **🔴 RE-505 = 5/30 に main.py 経路で単独本番稼働の痕跡** — audit_2026-05-30.jsonl に `account_balance=1,000,000` (main.py default) の PTRC reject 20 件、同日 UnifiedRunner ログ不在。これは long-pending な「二重実装」問題が顕在化した最重大インシデント候補。
2. **post_fix_defect の連鎖**: 前回 AUDIT-2026-0530-002 後にも post_fix_defect が 6 件確定 (CSR-601/602, PA-501/502, DE-403 ラベル意味論の incomplete fix)。「修正は適用されたが配線が漏れた」パターンが依然構造的に再生産されている。
3. **main.py vs runner.py 二重実装の負債が複数欠陥に波及**: RE-501/502/503/504 (PostPTRC, CircuitBreaker, LatencyMonitor, RetryManager) が UnifiedRunner で完全未配線、PA-503 が本番 LIVE 検知 5 種 dead を確認。DESIGN-D-001 / PA-407 として ADR 案件。

---

## ⚪ INVALIDATED (1 件、2026-06-01 ユーザー仕様確認)

### ~~RE-505~~ [INVALID] main.py 経路で 2026-05-30 単独本番稼働の痕跡 ← **誤推定**

**当初推定**: `audit_2026-05-30.jsonl` に `account_balance=1,000,000` PTRC reject 20 件 + 同日 `unified_runner.20260530.*` 不在 → main.py 単独本番稼働。

**ユーザー指摘 (2026-06-01)**: 「土日は市場が閉じているため OANDA の取引停止時間はログを止めるように変更した」**仕様**。週末ログ不在は意図的な動作であり、main.py 単独稼働の証拠ではない。

**仕様記録**: `fx_trading_system/CLAUDE.md` § 仕様メモ「週末・OANDA 取引停止時間中のログ抑制」セクションに耐久記録済。memory `feedback_weekend_log_suppression_spec.md` にも記録。

**残課題**: `account_balance=1,000,000` の出現自体は別途調査余地あり（dev/staging 経路 / ATLAS validation / backtest 等の可能性）が、CRITICAL 扱いは不適切と判定。RE-501/502/503/504/PA-503 (UnifiedRunner の wiring 不足) は code-level findings として独立に有効、severity 維持。

**訂正後 CC-001**: 「main.py 単独本番稼働 compound risk」仮説は撤回。ただし「main.py vs runner.py 二重実装 + UnifiedRunner で複数の安全機構が wiring されていない」事実は引き続き構造的負債として有効 (ADR-PA-307 / PA-407)。
...
| ID | 領域 | 概要 | 修正コスト |
|----|------|------|-----------|
| **CSR-601** | ATLAS path_safety | `validate_instrument`/`validate_timeframe` が dead code — `atlas data fetch` で書込パストラバーサル可能。CSR-302/303 修正が「関数追加のみで配線漏れ」 | 数行 |
| **CSR-602** | ATLAS path_safety | `atlas context improve --strategy-id` が `validate_generation_id` をバイパス — 5/8 サブコマンドのみ保護、CSR-303 漏れ | 数行 |
...
6 件の HIGH (PA-503/505/506, RE-501/502/503/504, DE-501/505, DM-501) のいずれもが main.py と runner.py の機能セット非対称に起因。ADR-PA-307 (main.py 廃止 or 共通モジュール抽出) の前倒し以外に根本解決手段なし。

### CC-004 — テスト基盤の構造的死角
QA-601/607 で示される通り、CSR-101 RCE の `ast.Name` ノード経由攻撃、main.py vs runner.py 配線対称性、E2E full-flow が回帰テストでカバーされていない。Phase 4.5 を活かすには契約テストの拡充が前提。
